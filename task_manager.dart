...
class _NotesTabState extends State<NotesTab> {
  ...
  void initState() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      // If the message contains a notification, display it
  ...
  void _showAddTaskDialog(BuildContext context) {
    ...
    Future<void> addTask(StateSetter setDialogState) async {
      ...
      try {
        ...
        final DateTime deadline = DateTime.parse(deadlineController.text);

        // Schedule notification 1 hour before the deadline
        final DateTime notificationTime = deadline.subtract(const Duration(hours: 1));

        if (notificationTime.isAfter(DateTime.now())) {
          await scheduleTaskNotification(
              DateTime.now().millisecondsSinceEpoch, // Generate unique ID based on timestamp
              'Task Reminder',
              'Your task "${titleController.text}" is due in 1 hour!',
              notificationTime,
            );
        }
...
Future<void> scheduleTaskNotification(int id, String title, String body, DateTime scheduledDate) async {
  await flutterLocalNotificationsPlugin.zonedSchedule(
    ...
    matchDateTimeComponents: DateTimeComponents.time, // Match only time if needed
  );
}
